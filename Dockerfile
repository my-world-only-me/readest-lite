...
#   2. 用 extract-runtime-deps.js 只提取运行时必需的生产依赖（~50-80MB）
...
# ── 🚀 提取运行时必需的生产依赖 ──────────────────────────────────────────
# 从 .pnpm store 中只提取 entrypoint 需要的包（prisma / argon2 /
# @prisma/client / jsonwebtoken）及其传递依赖，输出扁平 node_modules。
# 脚本见 docker/extract-runtime-deps.js
